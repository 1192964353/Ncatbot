...
class EpicFreeGamesPlugin(NcatBotPlugin):
    ...
    async def on_load(self):
        ...
        self.add_scheduled_task("EpicFreeGames", interval="9:30")

    async def EpicFreeGames(self):
        if weekday == 4:  # 星期五
            for group_id in GroupList:
                await self.api.qq.post_group_array_msg(group_id=group_id, MessageArray=await self.GetFreeGames())  # 你的发消息函数
