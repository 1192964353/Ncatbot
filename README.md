...
机器人启动后，`console_chat` 插件会在服务器终端提供控制台发消息入口。输入 `help` 查看命令；例如 `g 1061778880 服务器维护完成` 向指定群发送消息，`u 1192964353 测试消息` 发送私聊消息，`all 服务器维护将在 10 分钟后开始` 向机器人所在的所有群发送消息。输入 `quit` 只停止控制台监听，不会关闭机器人。
